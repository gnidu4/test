...
class YouTubeDownloaderMod(loader.Module):
    ...
    async def yt_cmd(self, message):
        ...
        try:
            ...
            def response_checker(event):
                return event.is_reply and event.reply_to_msg_id == message.id

            response = await self.client.wait_for(events.NewMessage(incoming=True, from_users=message.from_id), check=response_checker)
            quality_choice = int(response.message.strip()) - 1
