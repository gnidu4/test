...
class YouTubeDownloaderMod(loader.Module):
    ...
    async def yt_cmd(self, message):
        ...
        try:
            ...
            # Фильтруем форматы, чтобы оставить только те, которые содержат видео и аудио и имеют разрешение от 480p
            filtered_formats = [
                fmt for fmt in formats
                if 'video' in fmt['format'] and 'audio' in fmt['format'] and
